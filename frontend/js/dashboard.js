...
document.addEventListener("DOMContentLoaded", () => {
  load();
  ["searchComplaints", "statusFilter", "priorityFilter", "authorityFilter"].forEach(id => document.getElementById(id)?.addEventListener("input", render));
...
function populateAuthorityFilter() {
  ...
  const authorities = [...new Set(complaints.map(c => c.authority))].sort();
  sel.innerHTML = '<option value="">All authorities (super-admin view)</option>' +
    authorities.map(a => `<option value="${esc(a)}">${esc(a)}</option>`).join("");
  if (authorities.includes(current)) sel.value = current;
}
...
function render() {
  const q = document.getElementById("searchComplaints").value.toLowerCase(),
    ...
    auth = document.getElementById("authorityFilter").value;
  const a = complaints.filter(c => (!q || `${c.id} ${c.area} ${c.issue_type} ${c.description}`.toLowerCase().includes(q)) && (!s || c.status === s) && (!p || c.priority === p) && (!auth || c.authority === auth));
  document.getElementById("complaintsList").innerHTML = a.length ? a.map(c => `<div class="complaint" onclick="openComplaint('${esc(c.id)}')"><span class="id">${esc(c.id)}</span><div><b>${esc(c.issue_type)}</b><small>${esc(c.area)} • ${esc(c.department)}</small></div><span class="badges"><em class="${c.priority.toLowerCase()}">${esc(c.priority)}</em><em>${esc(c.status)}</em></span></div>`).join("") : '<div class="empty">No complaints found for this view.</div>'
}
